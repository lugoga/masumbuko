...
date: '2020-07-10'
slug: integrate-r-and-python-in-rstudio-to-power-your-analytical-capability
categories:
  - Python
tags:
  - programming
...
In this section we use function from R to read the dataset from the local machine into R session. The function `readxl::read_excel()` function is used [@readxl]. The outocome after reading the file is the *tibble* format---a modern data frame from the tidyverse ecosystem [@tidyverse].  We first load the tidyverse
...
```{r, eval=FALSE}
kgr = readxl::read_excel("dege.xlsx")
...
We then use the `mutate` function from **dplyr** [@dplyr] to create new column called `month` and then select `sst`, `chl`, and `month` variables.

```{r}
kg_r = kg_r%>% mutate(month = lubridate::month(time, abbr = T, label = T))%>% select(sst,chl, month) 
```

Then, we can visualize the dataset with **ggplot** package [@ggplot]. The chunk below was used to generate figure \@ref(fig:fig3)
