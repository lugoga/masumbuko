---
title: Obtain Oceanographic Data with R
...
**ERDDAP** is a web service developed by Bob Simons of NOAA wth are over sixty [ERDDAP servers](https://coastwatch.pfeg.noaa.gov/erddap/metadata/fgdc/xml/) providing access to a myriad satellite data and model output relevant to oceanography, meteorology, fisheries and marine mammals, among other areas. The good thing about **ERDDAP** is its ability to allow users define the area they interested and then subset data that is within the extent of the defined area. 
...
Where $2\Omega sin \varphi$ is the Coriolis pparamer. The equation can be mathematically writen as equation \@ref(eq:eq2)
...
The geostrophic approximation applied at $z = 0$ leads to a very simple relation--- surface geostrophic current are proportional to surface slope. For example, consider a level surface slightly below the se sea surface, say two meters below the sea surface , at $z = -r$ figure \@ref(eq:eq3) 
