...
Figure \@ref(fig:vector) show Surface current vectors compiled from drifters during the northeast (Figure \@ref(fig:vector)) and southeast monsoon season (Figure \@ref(fig:vector)). 
...
Barale^[Barale, V. (2014). The African Marginal and Enclosed Seas: An Overview. In V. Barale & M. Gade (Eds.), Remote Sensing of the African Seas (pp. 4-29): Springer Science.] described that the South Equatorial Current, flowing westward with the Trades north of 22 ${^{\circ}$S, divides into a north-flowing stream that feeds the northward East Africa Coastal Current, and a south-flowing stream that feeds the East Madagascar Current. The East African Coastal Current runs northward throughout the year between 11 {^{\circ}$S,nd 3 {^{\circ}$S. 
...
## northeast monsoon season
leaflet(data = drifter.traj%>%filter(season == "NE"))%>%
  addTiles()%>%
   addPolylines(color = "green",  weight = 0.75)%>%
  fitBounds(lng1 = 39.6, lat1 = -13, lng2 = 45.1, lat2 = -3)%>%
  addMeasure(position = "bottomleft",
...
leaflet(data = drifter.traj%>%filter(season == "SE"))%>%
  addTiles()%>%
   addPolylines(color = "green",  weight = 0.75)%>%
  fitBounds(lng1 = 39.6, lat1 = -13, lng2 = 45.1, lat2 = -3)%>%
  addMeasure(position = "bottomleft",
