...
```{r fig0, fig.cap="Conceptual model tidymodel's concept. Courtesy of Edgar Ruiz", echo=FALSE}
 ...
 cowplot::ggdraw() + cowplot::draw_image("tidymodel_concept.png", scale = 1)
```


# Data

We use the `dagaa` dataset to illustrate the concept. This dataset comes with base R as a data frame. This dataset consists of 50 samples from each of three species of fish (*Lates, Limno and Stolothrisa*). It also contains four variables--- the total length, weight, sex and stage of maturity that were collected to quantify the morphologic variation of three three species.  We can import the the file using `read_excel` function from **readxl** package and specify the sheet number where the data is stored and the number of locums to skip. 

```{r, eval=FALSE}
dagaa = readxl::read_excel("dagaa_species.xls",  sheet = 3, skip = 8) 
```


```{r, echo=FALSE}
dagaa = readxl::read_excel("e:/Data Manipulation/gillnet_kigoma/dagaa/Ringnet_research_Data_Feb2020.xls", 
                                 sheet = 3, skip = 8) 

dagaa.clean = dagaa%>%
  select(7,9:12) %>%
  rename(species = 1, total_length = 2, weight = 3, sex = 4, stage_mat = 5) %>% 
  mutate(total_length = as.integer(total_length), 
         stage_mat = replace(stage_mat, stage_mat == "2", 'I'),
         stage_mat = replace(stage_mat, stage_mat == "2A", 'I'),
         stage_mat = replace(stage_mat, stage_mat == "2B", 'II'),
         # stage_mat = replace(stage_mat, stage_mat == "M", 'III'), ## turn off comment and include the "M" 
         stage_mat = replace(stage_mat, stage_mat == "3", 'III'),
         stage_mat = replace(stage_mat, stage_mat == "4", 'IV'),
         stage_mat = replace(stage_mat, stage_mat == "5", 'IV'))   %>%
  mutate_if(is.character, as.factor)%>% group_by(species) %>%
...
### Gain and Roc Curve
When we develop statistical models for classification tasks (e.g. using machine learning), we usually need to have a way to compare the generated models to decide which model is best. Typical tools for this task are Gains, ROC or Lift charts.
