...
pemba%>%separate(date, c("date", "time"), sep = " ")%>%
  select(date, lon,lat, u, v, velocity)%>%
  sample_n(12)%>%
  kable("html", align = "c", digits = 2,
        caption = "Overview of the zonal, meridional and surface current velocity", 
        col.names = c("Date", "Longitude", "Latitude", "U", "V", "Velocity"))%>%
  kable_styling()%>%
  add_header_above(c("", "Location Information" = 2, "Surface Current" = 3))%>%
  add_header_above(c("" , "Surface Current Information" = 5))%>% 
  column_spec(1:6, width = "8cm")%>%
  column_spec(1, background = "lightpink", bold = F, color = 1)%>%
  column_spec(2:3, background = "lightgreen", bold = F, color = "black")%>%
  column_spec(4:6, background = "lightblue", bold = F, color = "black")
```



Figure \@ref(fig:current) show the seasonal difference of surface current in the Pemba channel. The southe east monsoon has more drifter observation and well distributed surface current (Figure \@ref(fig:current)b) compared to the northeast monsoon season (Figure \@ref(fig:current)a)
