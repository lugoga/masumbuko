---
title: Surface current from Drifters
...
We first make a rectangular grid over the bounding domain of the drifter observations> parsed the simpe point feature of drifter observation into the sf's function *st_make_grid()* and opted for 1600 grids (40 $\:\times \:$ 40). The grid generated was simple feature geometry list-column (sfc) format, which prevent computation. To make computation in this file, I converted from *sfc* to *sf* with *st_sf()* function. 
```{r}
...
the index and number of drifter contained in the grids were determined and then the median of zonal (U) meridional (V) components and velocity of each grid were computed. In this approach, the total number of observations within a grid were determined using equation \@ref(eq:obs). Once the number of observations were determined, only grids with more than four observation of drifters were selected to calculate the median zonal component in equation \@ref(eq:u), the median meridional component inequation \@ref(eq:v) and the median median velocity in equation \@ref(eq:velocity) for each grid.
...
Figure \@ref(fig:fig1)
```{r fig1, fig.cap="Density of observations (Number of drifter per grid"}
ggplot()+
...
Figure \@ref(fig:fig2)
```{r fig2, fig.cap="near-surface currents derived from satellite-tracked drifter trajectories"}
ggplot()+
...
Figure \@ref(fig:fig3)
```{r fig3, fig.cap="near-surface currents vector overlaid on surface current velocity", warning=FALSE, message=FALSE, comment=""}
